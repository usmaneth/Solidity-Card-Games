...
uint8 const STARTING_HAND_SIZE = 7;
...
// It's the next player's turn
  ...
  gameState.turn = (_playerIndex + gameState.direction) % players.length;
} else {
...
function shuffleDeck() private {
  for (uint8 i = 0; i < deck.length; i++) {
    uint8 j = uint8(keccak256(abi.encodePacked(block.number, now, msg.sender, i))) % deck.length;
